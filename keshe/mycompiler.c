...
ASTnode* program() {
    ...
    if (root->child = ExtDefList()) {printf("词法分析结束\n"); return root;}//语法正确返回语法树根节点
...
ASTnode* ExtVarDef() {
    ...
    while (1) {
        if (w != COMMA || w != SEMI) {printerror(row, col, "外部变量定义-格式错误\n"); /*AST_clear(r);*/ return NULL;}//报错，释放root为根的全部结点，空指针返回
...
ASTnode* statement() {
    ...
    switch (w) {
        ...
        case WHILE : //分析while循环语句，
            ...
            if (w != RP) {printf("fuck\n"); return NULL;}
            statement_statementBlock(); //用于保留循环体的子树指针
            w = gettoken();
            if (w != RP) {/*报错返回空结点*/}
